...
m1 <- lmer(peak.torque ~ time * supplement + (1|subject), 
           data = filter(rest.dat,
                         test == "isom"))
...
m2 <- lmer(peak.torque ~ time * supplement + (1|subject), 
           data = filter(rest.dat,
                         test == "isok.60"))
...
m3 <- lmer(peak.torque ~ time * supplement + (1|subject), 
           data = filter(rest.dat,
                         test == "isok.240"))
...
pairs(emmeans(m3, specs = ~ "time|supplement"), reverse = TRUE) %>%
        confint()
