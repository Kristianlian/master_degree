...
lpvol.plot <- confint.m1 %>%
        ...
        scale_x_discrete(labels=c("change.1" = "Baseline", "change.2" = "Session 2", "change.3" = "Session 3",
                                  ...
                                  "change.6" = "Session 6")) +
        labs(x = "", y = "LP training volume \n(kg change)\n", fill = "Supplement") +
        theme_classic() +
        #theme(plot.background = element_rect(fill = "gray80")) +
        theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
        theme(legend.position = "none")

saveRDS(lpvol.plot, "./data/derivedData/lpvol.plot.RDS")

# Knee extension
kevol.plot <- confint.m2 %>%
        data.frame() %>%
        add_row(supplement = "placebo", time = "change.1", emmean = 0, SE = 0, df = 0, lower.CL = 0, upper.CL = 0, .before =1) %>%
        add_row(supplement = "glucose", time = "change.1", emmean = 0, SE = 0, df = 0, lower.CL = 0, upper.CL = 0, .before =2) %>%
        ggplot(aes(time, emmean, group = supplement, fill = supplement)) +
        ...
        scale_x_discrete(labels=c("change.1" = "Baseline", "change.2" = "Session 2", "change.3" = "Session 3",
                                  "change.4" = "Session 4", "change.5" = "Session 5", 
                                  "change.6" = "Session 6")) +
        labs(x = "", y = "KE training volume \n(kg change)\n", fill = "Supplement") +
        theme_classic() +
        #theme(plot.background = element_rect(fill = "gray80")) +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))

saveRDS(kevol.plot, "./data/derivedData/kevol.plot.RDS")
...
plot_grid(lp.plot, ke.plot, ncol = 2, rel_widths = c(1,1.3))
# Bytt til ncol = 2 for å få figurene sidestilt
